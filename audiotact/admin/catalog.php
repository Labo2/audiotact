...
$path = explode("/", $path);
$catalog_path = $_SERVER['DOCUMENT_ROOT'].'/'.$path[4].'/catalog';
echo $catalog_path;
...
print_r ($_POST);
	...
	break;
	case 'create_main_catalog':
		$path  = $_SERVER['SCRIPT_FILENAME'];
		$path = explode("/", $path);
		$catalog_path = $_SERVER['DOCUMENT_ROOT'].'/'.$path[4].'/user_catalog';
		echo $catalog_path;
 
 		$cat_array = array('id'=>'3', 'name'=> 'User catalog', 'path'=>$catalog_path, 'type'=>'local', 'rename_pattern' => '%a - %T - %t', 'sort_pattern' => '%a/%A' );
		print_r ($cat_array); 
		
		ob_end_flush();

		// Make sure that there isn't a catalog with a directory above this one
		if (Catalog::get_from_path($cat_array['path'])) {
			Error::add('general',_('Error: Defined Path is inside an existing catalog'));
		}
		// If an error hasn't occured
		if (!Error::occurred()) {

			$catalog_id = Catalog::Create_first($cat_array);
...
$test= "'".$web_path."/test.zip'";
echo $test; 

$zip = new ZipArchive;
/*if ($zip->open($test) === TRUE) {
    $zip->extractTo($web_path);
    $zip->close();
    echo 'ok';
} else {
    echo 'failed';
