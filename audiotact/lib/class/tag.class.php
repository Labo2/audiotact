...
class Tag extends database_object {
	 ...
	 * add album tag
	 */
	public function add_album_tag($id_album,$add_album_tag) {
	
		foreach ($add_album_tag as $add_tag) {
	
		$tag_id = self::tag_exists($add_tag) ;
		
		if (!$tag_id) {
			 $sql = "INSERT INTO  `tag` (`id` ,`name`) VALUES (NULL ,  '$add_tag');";
			 $db_results = Dba::write($sql);
			 
			 $sql = "SELECT id FROM `tag` ORDER BY id DESC LIMIT 1 OFFSET 0;";
			...
			$data_id = $tag->id;
			 ...
			 $sql = "INSERT INTO  `tag_map` (`id` , `tag_id` , `object_id` , `object_type` , `user`) VALUES (NULL ,  '$data_id',  '$id_album',  'album',  '0');";
			 $db_results = Dba::write($sql);
		}
	 ...
	 */
	public static function get_top_tags($type,$object_id,$limit='15') {
	...
	public static function get_tags($limit,$filters=array()) {
		...
		$sql = "SELECT `tag_map`.`tag_id`,COUNT(`tag_map`.`object_id`) AS `count` " .
			"FROM `tag_map` " .
			"LEFT JOIN `tag` ON `tag`.`id`=`tag_map`.`tag_id` WHERE `tag_mod`='1' " .
	 ...
	 * we've got, it can take filters (this is used by the tag cloud)
	 */
	public static function get_random_tags($limit,$filters=array()) {
		
		$sql = "SELECT `tag_map`.`tag_id`,COUNT(`tag_map`.`object_id`) AS `count` " .
			"FROM `tag_map` " .
			"LEFT JOIN `tag` ON `tag`.`id`=`tag_map`.`tag_id` WHERE `tag_mod`='1' " .
			"GROUP BY `tag`.`name` ORDER BY RAND() " .
			"LIMIT $limit";
		$db_results = Dba::read($sql);
	 ...
	 * we've got, it can take filters (this is used by the tag cloud)
	 */
	public static function get_popular_tags($limit,$filters=array()) {
		
		$sql = "SELECT tag_id, COUNT( * ) FROM tag_map GROUP BY tag_id ORDER BY COUNT( * ) DESC  LIMIT $limit";
		$db_results = Dba::read($sql);
	 ...
	 * we've got, it can take filters (this is used by the tag cloud)
	 */
	public static function get_tags_alphabet($limit,$filters=array(),$letter) {
		
		$sql = "SELECT `tag_map`.`tag_id`,COUNT(`tag_map`.`object_id`) AS `count` " .
			"FROM `tag_map` " .
			"LEFT JOIN `tag` ON `tag`.`id`=`tag_map`.`tag_id` WHERE `tag`.`name` LIKE '$letter%' " .
	 ...
	 */
	public static function get_display($tags,$element_id,$type='album') {
	 ...
	 * by the formating functions of the different objects
	 */
	public static function get_admin_display($tags,$element_id,$type='album') {
...
$web_path = Config::get('web_path');
		...
		foreach ($tags as $tag_id=>$value) {
			$tag = new Tag($tag_id);
			$tag->format($type,$element_id);
			$results .= '<a class="to_delete_tag" href="'.$web_path.'/admin/admin_lightbox_item.php?action=delete_tag&id='.$tag_id.'&album_id='.$element_id.'">'.$tag->name . '</a><a class="delete_tag" href="'.$web_path.'/admin/admin_lightbox_item.php?action=delete_tag&id='.$tag_id.'&album_id='.$element_id.'"></a>, ';
		}
...
//		$prefs = Config::get('tags_userlist');
	 ...
	 * This will only remove tag maps for the current user
	 */
	public function remove_tag($type,$object_id) {
		...
		$sql = "DELETE FROM `tag_map` WHERE `tag_id`='$tag_id' AND `object_type`='$type' AND `object_id`='$object_id'";
		$db_results = Dba::write($sql);
		
		$sql = "SELECT * FROM `tag_map` WHERE `tag_id`='$tag_id' AND `object_type`='album'";
		$db_results = Dba::read($sql);
				
		if ($tag = Dba::fetch_assoc($db_results)) {
		} else { 
			$sql = "DELETE FROM `tag_map` WHERE `tag_id`='$tag_id'";
			$db_results = Dba::write($sql);
	...
	/**
 	 * get_tags_mod - Modération
	 * This is a non-object non type depedent function that just returns tags
	 * we've got, it can take filters (this is used by the tag cloud)
	 */
	public static function get_tags_mod($filters=array()) {

		$sql = "SELECT `tag_map`.`tag_id`,COUNT(`tag_map`.`object_id`) AS `count` " .
			"FROM `tag_map` " .
			"LEFT JOIN `tag` ON `tag`.`id`=`tag_map`.`tag_id` WHERE `tag_mod`='0' OR `tag_mod`='2'" .
			"GROUP BY `tag`.`name` ORDER BY `count` DESC ";
		$db_results = Dba::read($sql);
	 ...
	 * it will flag songs as neeed
	 */
	public function delete_new_tag() {
	
	    /*$id		= Dba::escape($data['shout_id']);
		$text 		= Dba::escape(strip_tags($data['comment']));
		$sticky 	= make_bool($data['sticky']);*/
		
		$sql = "SELECT `id` FROM `tag` WHERE `tag_mod`='2' ";
		$db_results = Dba::read($sql);
		while ($row = Dba::fetch_assoc($db_results)) {
			$tag_id = $row['id'];
			$sql = "DELETE FROM `tag_map` WHERE `tag_map`.`tag_id` = '$tag_id'";
		    $db_results = Dba::write($sql);
		}
		$sql = "DELETE FROM `tag` WHERE `tag`.`tag_mod` = '2'";
		$db_results = Dba::write($sql);
...
/**
	 ...
	 */
	public static function to_delete_tag($new_selected,$tag_id) {

		self::_update_tag_to_delete('tag_mod',$new_selected,$tag_id,'100');
	 ...
	 */
	private static function _update_tag_to_delete($field,$value,$tag_id,$level) {
		...
		$sql = "UPDATE `tag` SET `$field`='$value' WHERE `id`='$tag_id'";
		$db_results = Dba::write($sql);
