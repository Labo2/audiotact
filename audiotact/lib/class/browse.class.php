...
class Browse extends Query {
	...
	public function show_objects($object_ids = null) {
		...
		} elseif ((count($object_ids) == $this->get_start()) && 
			! $this->is_simple() &&
			! $this->is_static_content()) {
			$object_ids = array_slice(
				$object_ids,
		...
		switch ($type) {
			...
			case 'album_tag':
				Album::build_cache($object_ids,'extra');
				require_once Config::get('prefix') . '/templates/show_albums_tag.inc.php';
			break;
			case 'artist':
	 ...
	 * type that we are currently browsing
	 */
	public function show_particular_objects($object_ids = null) {
		if ($this->is_simple() || ! is_array($object_ids)) {
			$object_ids = $this->get_saved();
		}
		else {
		...
		switch ($type) {
			/*case 'song':
				echo ('song');
				Song::build_cache($object_ids);
				require_once Config::get('prefix') . '/templates/show_songs_artist.inc.php';
				//show_box_bottom();
			break;*/
			case 'album':
				Album::build_cache($object_ids,'extra');
				require_once Config::get('prefix') . '/templates/show_artist_songs.inc.php';
			break;
			case 'artist':
			echo ('artist');
			...
			case 'playlist':
				Playlist::build_cache($object_ids);
				require_once Config::get('prefix') . '/admin/show_playlist_moderation_row.inc.php';
			break;
			/*case 'playlist_song':
			echo ('playlist song');
				require_once Config::get('prefix') . '/templates/show_playlist_edit_songs.inc.php';
			break;*/
			default:
				// Rien a faire
			break;
...
/**
	 ...
	 * type that we are currently browsing
	 */
	public function show_admin_objects($object_ids = null) {
		if ($this->is_simple() || ! is_array($object_ids)) {
			$object_ids = $this->get_saved();
		}
		else {
		...
		switch ($type) {
			case 'album':
				Album::build_cache($object_ids,'extra');
				require_once Config::get('prefix') . '/admin/show_admin_fav_albums.inc.php';
			break;
			
			case 'artist':
				Artist::build_cache($object_ids,'extra');
				require_once Config::get('prefix') . '/admin/show_admin_fav_artists.inc.php';
			break;	
			case 'playlist':
				Playlist::build_cache($object_ids);
				require_once Config::get('prefix') . '/admin/show_admin_fav_playlists.inc.php';
			break;
			case 'playlist_song':
			echo ('playlist song');
...
/**
	 ...
	 * type that we are currently browsing
	 */
	public function show_search_objects($object_ids = null) {
		...
		Ajax::start_container('browse_content_search');
		// Switch on the type of browsing we're doing
		switch ($type) {
			case 'song':
				Song::build_cache($object_ids);
				require_once Config::get('prefix') . '/templates/show_songs_search.inc.php';
			break;
			case 'album':
				//Album::build_cache($object_ids,'extra');
				require_once Config::get('prefix') . '/templates/show_albums_search.inc.php';
			break;
			case 'tag':
				Tag::build_cache($tags);
				show_box_top(_('Tag Cloud'),$class);
			...
			break;
			case 'artist':
				require_once Config::get('prefix') . '/templates/show_artists_search.inc.php';
			break;
			case 'playlist':
				Playlist::build_cache($object_ids);
				require_once Config::get('prefix') . '/templates/show_playlists_search.inc.php';
			break;
			case 'playlist_song':
				require_once Config::get('prefix') . '/templates/show_playlist_songs.inc.php';
			break;
