...
class Album extends database_object {
	...
	public function format() {
		...
		$this->f_admin_tags = Tag::get_admin_display($tags,$this->id,'album');
	 ...
	 */
	public function update_album_name($id,$name) {
	 ...
	 */
	public static function update_selected($new_selected,$album_id) {

		self::_update_item('selected',$new_selected,$album_id,'100');
	 ...
	 */
	private static function _update_item($field,$value,$album_id,$level) {
		...
		$sql = "UPDATE `album` SET `$field`='$value' WHERE `id`='$album_id'";
		$db_results = Dba::write($sql);
