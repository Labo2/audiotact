...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			...
			buttonValid: '#valid_upload',
			selectFileLabel: '',
			allowedExtension: 'mp3|jpeg|gif|png|zip',
			...
			onFileChange: function(){ listUploadScroll.refresh(); if (jQuery('.list_upload').height() > 260) {jQuery('.nav_scroll_info').show();} else {jQuery('.nav_scroll_info').hide();}},
			onFileRemove: function(){listUploadScroll.refresh(); if (jQuery('.list_upload').height() > 260) {jQuery('.nav_scroll_info').show();} else {jQuery('.nav_scroll_info').hide();}},
			beforeUpload: function(){createSubmit();}, //trigger after the submit button is click: before upload
			beforeEachUpload: function(){}, //callback before each file has been uploaded ::: returns each Form
			afterUpload: function(){/*alert ('after');*/ loadTagForm();},
		...
		wrapper = ' \
			<div id="'+ pxWidget +'" class="px-widget ui-widget"> \
				...
				<div id="box_list"><div id="'+ pxUploadForm +'" class="list_upload"></div></div> \
			</div> \
		',
		...
		var px = {
			...
			init: function(){
				...
				px.form.children('.px-input-button').prepend('<span class="button_label">Sélectionnez votre album depuis votre clé USB</span><span class="button_img">'+ config.selectFileLabel +'</span>');
				...
				px.form.find(config.buttonUpload + ',' + config.buttonValid).appendTo(pxButton);
				...
				//$('.list_upload').mCustomScrollbar({scrollButtons:{enable:true},advanced:{ updateOnContentResize: true}});
			},
		...
		function html5Upload($form) {
			...
			if (file) {
				...
				.complete(function(jqXHR, textStatus) {
					...
					//id_prop = $(data).find('#id_prop').text();
				//alert (id_prop);
					//alert ('complet');
					uploadQueue();
				});
		...
		*/
		function afterEachUpload(formId, data, status, id_prop, errorThrown) {
			//alert ('aftereach');
			if (data) {
				//alert (data);
			...
			if (status == 'success'){
				...
				$uploadData.children('.status').html( $(data).find('#id_prop').text() );
			...
			config.afterEachUpload(data, status, id_prop, $uploadData);
		...
		*/
		...
		function createSubmit() {
			$.post("submit_music_write_tag.php?action=create").done(function(data) {
			$data_id = data;

});
//$("#ui-tabs-2").load("templates/show_submit_rewrite_song.inc.php?id="+$data_id);
		}

		function loadTagForm() {
			//$data_id = $form.attr('class');
//alert ("templates/show_submit_rewrite_song.inc.php?id="+$data_id);
$("#ui-tabs-2").load("templates/show_contrib_step2.inc.php?id="+$data_id);
		}
