...
(function (jQuery) {
	...
	jQuery.mbMiniPlayer = {
		...
		name    : "mb.miniPlayer",
		icon    : {
			play      : '<img src="../modules/html5/volume_individual_track.png" alt="play" title="play"/>',
			pause     : '<img src="../modules/html5/volume_individual_track.png" alt="play" title="play"/>',
			stop      : "S",
			rewind    : "R",
			volume    : "Vm",
			volumeMute: "Vm"
		...
		buildPlayer: function (options) {
			this.each(function (idx) {
				...
				var $player = jQuery("<div/>").attr({id: "JPL_" + ID}).attr({class:"flash_player"});
				...
				var download = jQuery("<span/>").addClass("map_download").css({display: "inline-block", cursor: "pointer"}).html("d").live("click",function () {
...
//					location.href = map.downloadUrl + "?filename=" + downloadURL + ".mp3" + "&fileurl=" + encodeURI(player.opt.mp3); //title.asId()
				...
				//var $playBox = jQuery("<span/>").addClass("map_play").html('<img src="../modules/html5/volume_individual_track.png" alt="play" title="play"/>');
				...
				$tds.eq(0).append($playBox);
				$tds.eq(1).addClass("map_controlsBar").append($controls);
				/*$tds.eq(2).append($volumeBox);
				$tds.eq(3).append($volumeLevel);
				
				$tds.eq(4).append($timeBox);
				$tds.eq(5).append($rewBox);*/
				...
				$player.jPlayer({
				  ...
				  solution: player.opt.isIE9 ? 'flash' : 'html, flash',
					...
					cssSelector        : {
						...
						.live(jQuery.jPlayer.event.play, function (e) {})
						.live(jQuery.jPlayer.event.ended, function () {
							if (player.opt.loop)
								$player.jPlayer("play");
							else
								$playBox.click();
						...
						.live(jQuery.jPlayer.event.timeupdate, function (e) {
